...
def portmap_discover(target, port=135):
    ...
    for entry in entries:
        ...
        try:
            endpoints[_transport]
        except Exception:
            endpoints[_transport] = {}

        try:
            endpoints[_transport][uuid]
        except Exception:
            endpoints[_transport][uuid] = set()
    ...
    reporter.print_info(
        "DCERPC portmapper discovered ports: %s" % ",".join(list(map(str, ports)))
    )
    return endpoints
...
def can_bind_to_interface_on_port(target, port, credentials, uuid, version):
    ...
    reporter.print_in_progress(
        "Connecting to %s ... " % ncacn_target, prefix="         ", end="", verbose=True
    )
    try:
        dce.connect()
    except Exception as e:
