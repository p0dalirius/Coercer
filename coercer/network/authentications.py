...
def trigger_and_catch_authentication(
    options,
    dcerpc_session,
    target,
    method_trigger_function,
    listenertype,
    listen_ip=None,
    http_port=80,
):
    ...
    if listenertype not in ["smb", "http"]:
        ...
        with ThreadPoolExecutor(max_workers=3) as tp:
            ...
            if listenertype == "smb":
                ...
                tp.submit(
                    listener_instance.start_http, control_structure, http_port=http_port
                )
        ...
        if control_structure["result"] == TestResult.NO_AUTH_RECEIVED:
            try:
                control_structure["result"] = TestResult.from_string(
                    str(result_trigger._result)
                )
            except Exception:
                pass
