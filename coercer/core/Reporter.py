...
class Reporter(object):
    ...
    def __init__(self, options, verbose=False):
        ...
        if options.log_file is not None:
            logging.basicConfig(
                ...
                level=options.minimum_log_level,
    ...
    def print_testing(self, msprotocol_rpc_instance, **kwargs):
        self.print(
            "(",
            ("-testing-", EscapeCodes.BRIGHT_YELLOW),
            ") ",
            msprotocol_rpc_instance,
            prefix="      ",
            symbol=(">", EscapeCodes.BRIGHT_YELLOW),
            **kwargs,
        )
    ...
    def print_in_progress(self, prefix, message, **kwargs):
        self.print(
            message, prefix=prefix, symbol=(">", EscapeCodes.BRIGHT_YELLOW), **kwargs
        )

    @parse_print_args
    def print_info(self, prefix, message, **kwargs):
        self.print(
            message, prefix=prefix, symbol=("info", EscapeCodes.BRIGHT_GREEN), **kwargs
        )

    @parse_print_args
    def print_ok(self, prefix, message, **kwargs):
        self.print(
            message, prefix=prefix, symbol=("+", EscapeCodes.BRIGHT_GREEN), **kwargs
        )
    ...
    def print_warn(self, prefix, message, **kwargs):
        self.print(
            message,
            prefix=prefix,
            symbol=("warn", EscapeCodes.BRIGHT_RED),
    ...
    def print_result(
        self, symbol, result, msprotocol_rpc_instance, escape_code=None, **kwargs
    ):
        self.print(
            "(",
            (result, escape_code),
    ...
    def report_test_result(
        ...
        if (
            namedpipe
            not in self.test_results[target][uuid][version][function_name].keys()
        ):
        ...
        if self.options.mode in ["scan", "fuzz"]:
            if (
                result == TestResult.SMB_AUTH_RECEIVED
                or result == TestResult.SMB_AUTH_RECEIVED_NTLMv1
                or result == TestResult.SMB_AUTH_RECEIVED_NTLMv2
            ):
                self.print_result(
                    "+",
                    "SMB Auth",
                    msprotocol_rpc_instance,
                    EscapeCodes.BOLD_BRIGHT_GREEN,
                )
                ...
                self.print_result(
                    ...
                    "-No Func-",
                    msprotocol_rpc_instance,
                    EscapeCodes.BOLD_BRIGHT_MAGENTA,
                )
            else:
                if self.verbose:
                    self.print_result(
        ...
        for target in self.test_results.keys():
            for uuid in self.test_results[target].keys():
                for version in self.test_results[target][uuid].keys():
                    for function_name in self.test_results[target][uuid][
                        version
                    ].keys():
                        for namedpipe in self.test_results[target][uuid][version][
                            function_name
                        ].keys():
                            for test_result in self.test_results[target][uuid][version][
                                function_name
                            ][namedpipe]:
                                data = [
                                    target,
                                    ...
                                    test_result["function"]["name"],
                                    test_result["function"]["opnum"],
                                    test_result["testresult"],
                                    test_result["exploitpath"],
                                ]
    ...
    def exportSQLITE(self, filename):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS results(target VARCHAR(255), uuid VARCHAR(255), version VARCHAR(255), named_pipe VARCHAR(255), protocol_shortname VARCHAR(255), protocol_longname VARCHAR(512), function_name VARCHAR(255), result VARCHAR(255), path VARCHAR(512));"
        )
            ...
            for uuid in self.test_results[target].keys():
                for version in self.test_results[target][uuid].keys():
                    for function_name in self.test_results[target][uuid][
                        ...
                        for named_pipe in self.test_results[target][uuid][version][
                            function_name
                        ].keys():
                            for test_result in self.test_results[target][uuid][version][
                                function_name
                                ...
                                cursor.execute(
                                    ...
                                    (
                                        ...
                                        function_name,
                                        test_result["testresult"],
                                        str(bytes(test_result["exploitpath"], "utf-8"))[
                                            2:-1
                                        ].replace("\\\\", "\\"),
                                    ),
                                )
